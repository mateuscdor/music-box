...
exports.addinfo = async (audio, image = '', title = '', author = '', description = '') => {
try {
...
exports.generateListYTS = async (search) => {
try {
if (search.length < 1) return 'no-res';
let yt = search.videos;
let listbutton = [];
let startnum = 1;
for (var x of yt) {
let button = {
    title: 'Result - ' + startnum++ + ' ',
    rows: [{
        title: x.title,
        rowId: `.song ${x.url}`
    }]
